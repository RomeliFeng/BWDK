/*
 * Protocol.cpp
 *
 *  Created on: 2017年5月8日
 *      Author: Romeli
 */

#include "Protocol.h"
#include "U_USART1.h"
#include "Temp.h"
#include "Relay.h"
#include "Setting.h"

#define Salve_Off 0
#define Fun_Off 1
#define Add_Off 3
#define Len_Off 5

uint16_t GetCRC16(uint8_t *ptr, uint8_t len);
void FormatSend(uint8_t fun, uint8_t* data, uint8_t dataLen);

//CRC 高位字节值表
const uint8_t TabH[] = {  };

//CRC 低位字节值表
const uint8_t TabL[] = {  };

void Serial_Event() {
	uint8_t buf[20];
	uint8_t buflen = Serial.available();
	Serial.read(buf, buflen);
	if (buf[Salve_Off] != MainSetting.setting.S_Add)
		return;
	if (GetCRC16(buf, buflen - 2)
			!= ((uint16_t) buf[buflen - 2] << 8 | buf[buflen - 1])) {
		return;
	}
	switch (buf[Fun_Off]) {
	case 0x03: {
		uint8_t data[8];
		uint8_t index = 0;
		for (int i = 0; i < buf[Len_Off]; ++i) {
			data[index++] = (uint8_t) (Temp::Data[buf[Add_Off] + i] >> 8);
			data[index++] = (uint8_t) Temp::Data[buf[Add_Off] + i];
		}
		FormatSend(buf[Fun_Off], data, index);
		break;
	}
	case 0x04:
		uint8_t data[2];
		data[0] = 0x00;
		data[1] = RelayStatus.byte;
		FormatSend(buf[Fun_Off], data, 2);
		break;
	default:
		break;
	}
}

uint16_t GetCRC16(uint8_t *ptr, uint8_t len) {
	uint16_t index;
	uint8_t crch = 0xFF; //高 CRC 字节
	uint8_t crcl = 0xFF; //低 CRC 字节

	while (len--) { //计算指定长度的 CRC
		index = crch ^ *ptr++;
		crch = crcl ^ TabH[index];
		crcl = TabL[index];
	}
	return ((crch << 8) | crcl);
}

void FormatSend(uint8_t fun, uint8_t* data, uint8_t dataLen) {
	uint8_t sendBuf[20];
	uint8_t index = 0;
	uint16_t crc;
	sendBuf[index++] = MainSetting.setting.S_Add;
	sendBuf[index++] = fun;
	sendBuf[index++] = dataLen;
	for (uint8_t i = 0; i < dataLen; ++i) {
		sendBuf[index++] = data[i];
	}
	crc = GetCRC16(sendBuf, index);
	sendBuf[index++] = crc >> 8;
	sendBuf[index++] = (uint8_t) crc;
	Serial.print(sendBuf, index);
}
